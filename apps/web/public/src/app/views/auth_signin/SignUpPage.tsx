...
const SignUpPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      toast(
        new ToastEntity('ลงทะเบียนสำเร็จ', 'คุณลงทะเบียนสำเร็จแล้ว', 'success')
      );
      navigate('/');
    } catch (error) {
  ...
  return (
    <div className="flex max-w-screen-lg w-full h-[600px] bg-white shadow-md rounded-md m-24 overflow-clip">
      <div className="text-white w-full max-w-96 overflow-hidden relative flex flex-col justify-center text-center items-center gap-10 px-16 bg-blue-400 bg-opacity-70 shadow-md">
        <h2 className="z-20 text-primary-500">สำหรับผู้มีบัญชี</h2>
        <p className="z-20 text-primary-500">ลงชื่อเข้าใช้เพื่อจองการทดสอบ</p>
        <Link to="/auth/signin" className="z-20">
          <Button
            variant="default"
